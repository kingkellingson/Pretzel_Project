...
public class OneChangeAdditionAndSubtraction {
    ...
    public static void main(String[] args) {
        ...
        int[] toAddTo = new int[] {-1,-1,-1,-1};
        int[] toTakeFrom = new int[] {-1,-1,-1,-1};
        int[] numsInEquation = new int[] {-1,-1,-1,-1};
        int additionResult;
        int subtractionResult = -1;
        boolean foundChange;
        ArrayList<String[]> onPage;
        ...
        for (int j = 0; j < 10; ++j) {
            ...
            checkChanges(toAddTo, toTakeFrom, j, 0, numsInEquation);
            for (int k = 0; k < 10; ++k) {
                if (k == 2 || k == 4) continue;

                checkChanges(toAddTo, toTakeFrom, k, 1, numsInEquation);

                additionResult = j + k;
                checkChanges(toAddTo, toTakeFrom, additionResult, 2, numsInEquation);

                onPage = new ArrayList<>();
                BuildEquation(j, k, additionResult, -1, onPage, true);

                //at this point I know which ones to add a pretzel to and which ones to take a pretzel from
                MatchAddPretzelWithTakeAwayPretzel(toAddTo, toTakeFrom, numsInEquation, onPage, true);

                if (j>k) {//meaning I can do a subtraction problem and still get a positive number.
                    ...
                    subtractionResult = j - k;
                    checkChanges(toAddTo, toTakeFrom, subtractionResult, 2, numsInEquation);

                    onPage = new ArrayList<>();
                    BuildEquation(j, k, subtractionResult, -1, onPage, false);

                    //at this point I know which ones to add a pretzel to and which ones to take a pretzel from
                    MatchAddPretzelWithTakeAwayPretzel(toAddTo, toTakeFrom, numsInEquation, onPage, false);
            ...
            toAddTo = new int[] {-1,-1,-1,-1};
            toTakeFrom = new int[] {-1,-1,-1,-1};
            numsInEquation = new int[] {-1,-1,-1,-1};
        }
    }

    private static void MatchAddPretzelWithTakeAwayPretzel(int[] toAddTo, int[] toTakeFrom, int[] numsInEquation, ArrayList<String[]> onPage, boolean doAdd) {
        boolean foundChange;
        foundChange = false;
        for (int l = 0; l < 4; ++l) {
            for (int m = 0; m < 4; ++m) {
                ...
                if (toAddTo[l] != -1 && toTakeFrom[m] != -1) {
                    ...
                    doChange(toAddTo[l], "addPretzel", changedNumsInEquation, l);
                    doChange(toTakeFrom[m], "takeAwayPretzel", changedNumsInEquation, m);
                    BuildEquation(changedNumsInEquation[0], changedNumsInEquation[1], changedNumsInEquation[2], changedNumsInEquation[3], onPage, doAdd);
    ...
    private static void checkChanges(int[] toAddTo, int[] toTakeFrom, int currNum, int currPos, int[] numsInEquation) {
        int count = 0, test = currNum, toHold = 0;
        do {
            if (currNum >= 10) {
                toHold = currNum % 10;
                currNum = currNum / 10;
            ...
            if (currNum == 0 || currNum == 1 || currNum == 3 || currNum == 5 || currNum == 6 || currNum == 9) {
                toAddTo[currPos+count] = currNum;
            }
            if (currNum == 6 || currNum == 7 || currNum == 8 || currNum == 9) {
                toTakeFrom[currPos+count] = currNum;
            }
            numsInEquation[currPos+count] = currNum;
            currNum = toHold;
            count++;
            test = test / 10;
        } while (test > 0);
    ...
    private static void doChange(int toChange, String type, int[] numsInEquation, int location) {
        ...
        if (type.equals("takeAwayPretzel")) {
            switch (toChange) {
    ...
    private static void BuildEquation(int int1, int int2, int result, int result2, ArrayList<String[]> onPage, boolean doAdd) {
        ...
        if (result2 != -1) {
            ...
            onPage.add(numToPretzel.get(result2));
        } else {
            if (result >= 10) {
